...
const generateImageUrl = () => {
	const { username, playmode, background, hexColor, version, height, supporter, team } = getFormValues();
	if (!username) return "";
...
const getFormValues = () => ({
	...
	height: elements.inputs.height.value || (elements.inputs.version.value === "full" ? 200 : 120),
...
elements.inputs.background.addEventListener("change", () => {
	elements.inputs.hexColor.disabled = elements.inputs.background.value !== "color";
